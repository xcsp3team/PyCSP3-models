...
if with_scenarii:
    first_scen = decrement(number_in(line()))
    last_scen = decrement(number_in(next_line()))
...
if with_scenarii:
    weights = numbers_in(next_line())
    assert len(weights) == nbScen
    durations = [numbers_in(next_line()) for _ in range(nbScen)]
    assert all(nOptions == len(t) for t in durations)
else:
...
if with_scenarii and next_line():
    last_scen = decrement(number_in(line()))  # to update it
if with_scenarii:
    data["scenarios"] = OrderedDict([("first", first_scen), ("last", last_scen), ("weights", weights), ("durations", durations)])
