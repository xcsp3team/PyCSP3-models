...
conflicts = [(i, j, v) for i, j in combinations(nSessions, 2) if (v := len(set(session_groups[i]).intersection(set(session_groups[j])))) > 0]
...
def range_no():
    g = sum(sum(slot_capacities[: maxSlots[i]]) for i in range(nSessions)) - sum(nSessionPapers)
...
def similar_sessions():
    avoidable_groups = [g for g in range(nGroups) if len([i for i in range(nSessions) if g in session_groups[i]]) == 1]
    m = [[g for g in session_groups[i] if g not in avoidable_groups] for i in range(nSessions)]
    return [(i, j) for i, j in combinations(nSessions, 2) if nSessionPapers[i] == nSessionPapers[j] and m[i] == m[j]]


# x[i][k] is the kth slot used for session i (NO if unused slot)
x = VarArray(size=[nSessions, maxSlots], dom=lambda i, k: range(nSlots + 1) if k < maxSlots[i] else None)

# y[i][k] is the number of papers presented in the kth slot used for session i (0 if unused slot)
y = VarArray(size=[nSessions, maxSlots], dom=lambda i, k: [0] + papers_range if k < maxSlots[i] else None)

satisfy(

    # channeling variables
    [(x[i][k], y[i][k]) in T for i in range(nSessions) for k in range(maxSlots[i])],

    # allocating the right number of papers for each session
    [Sum(y[i]) == nSessionPapers[i] for i in range(nSessions)],

    # not exceeding the possible number of parallel sessions
    Cardinality(
        within=x,
        occurrences={t: range(min_nb_rooms[t], nRooms + 1) for t in range(nSlots)} | {NO: range_no()}
    ),
    ...
    [
        If(
            x[i][k + 1] != NO,
            Then=x[i][k] < x[i][k + 1]
        ) for i in range(nSessions) for k in range(maxSlots[i] - 1)
    ],
