...
y = VarArray(size=[m, k], dom=range(1, n + 1))

z = VarArray(size=[m, k], dom=range(n, n * n + 1))
...
satisfy(
    ...
    [
        [y[i][j] == inv[x[i][j]] for i in range(m) for j in range(k)],

        [z[i][j] == y[i][j] * n for i in range(m) for j in range(k)],

        [AllDifferent([ftab[z[i1][j1] + y[i2][j2]] for j1 in range(k) for i2 in range(m) if i1 != i2 for j2 in range(k)]) for i1 in range(m)]
    ]
)

"""
1) This is an approximation of the initial problem, just used for the 2025 competition (Mini track)
"""
