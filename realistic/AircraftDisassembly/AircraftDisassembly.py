...
skill_capacities = [sum(skill_mastery[:, s]) for s in range(nSkills)]  # capacity of each skill
consumption = [
    [(required_mass[j] if locations[j] in comp_prod[i, 0] else -required_mass[j] if locations[j] in comp_prod[i, 1] else 0) for j in range(nActivities)]
    for i in M]
resAct = [[i for i in range(nActivities) if consumption[m][i] != 0] for m in M]  # set of "non-zero" activities for each mass balanced
...
satisfy(
    ...
    [
        ...
        ) for u, (i, j) in enumerate(unrelated_precedences)
        if any(skill_requirements[i][s] + skill_requirements[j][s] > skill_capacities[s] for s in range(nSkills))
    ],
    ...
    [
        ...
        if not any(skill_requirements[i][s] + skill_requirements[j][s] > skill_capacities[s] for s in range(nSkills))
    ],
    ...
    [
        ...
        ) for i in range(nUnavailable) for a in range(nActivities)
    ],

    # Skill constraint: Skill requirements are satisfied
    [Sum(contrib[a][r][s] for r in useful_resources[a]) == skill_requirements[a][s] for a in range(nActivities) for s in range(nSkills) if
     skill_requirements[a][s] > 0],

    # Non-Multi-Tasking constraint: Maximum of one contribution by each activity
    [Sum(contrib[a][r][s] for s in range(nSkills) if skill_mastery[r][s] == 1 and skill_requirements[a][s] > 0) <= 1 for a in range(nActivities) for r in
     useful_resources[a]],

    # Skill constraint: Resources only use skills they have mastered
    [contrib[a][r][s] <= skill_mastery[r][s] for a in range(nActivities) for r in useful_resources[a] for s in range(nSkills)],

    # Linking constraint: resources only contribute to activities they are assigned
    [contrib[a][r][s] <= assign[a][r] for a in range(nActivities) for r in useful_resources[a] for s in range(nSkills) if skill_requirements[a][s] > 0],

    # Location constraint: limit the maximum number of technicians allowed to work there at the same time.
    [Cumulative(origins=x, lengths=durations, heights=occupancies) <= location_capacities[i] for i in range(nLocations)],

    [act_leq_act[i][j] == (x[i] <= x[j]) for i in range(nActivities) for j in range(nActivities)],
    ...
    [either(act_leq_act[i][j], act_leq_act[j][i]) for i, j in combinations(nActivities, 2)],
)

minimize(
    100000 * Maximum(x) + Sum(costs[r] * durations[a] * assign[a][r] for a in range(nActivities) for r in range(nResources))
)
