...
satisfy(
    # computing x
    [x[i][j] == ift(shaded[i][j], 0, clues[i][j]) for i in range(n) for j in range(n)],

    # if two identical clues are adjacent then all equal clue values in the row/col have to be shaded
    [
        [[shaded[i][k] == 1 for k in range(n) if k not in (j, j + 1) and clues[i][k] == clues[i][j]] for i in range(n) for j in range(n - 1) if
         clues[i][j] == clues[i][j + 1]],
        [[shaded[k][j] == 1 for k in range(n) if k not in (i, i + 1) and clues[k][j] == clues[i][j]] for i in range(n - 1) for j in range(n) if
         clues[i][j] == clues[i + 1][j]]
    ],

    #  a cell between two identical cells cannot be shaded
    [
        [shaded[i][j] == 0 for i in range(1, n - 1) for j in range(n) if clues[i - 1][j] == clues[i + 1][j]],
        [shaded[i][j] == 0 for i in range(n) for j in range(1, n - 1) if clues[i][j - 1] == clues[i][j + 1]]
    ],
    ...
    [
        [AllDifferent(x[i], excepting=0) for i in range(n)],
        [AllDifferent(x[:, j], excepting=0) for j in range(n)]
    ],

    # no adjacent shaded cells
    [
        [either(shaded[i][j] == 0, shaded[i][j + 1] == 0) for i in range(n) for j in range(n - 1)],
        [either(shaded[i][j] == 0, shaded[i + 1][j] == 0) for i in range(n - 1) for j in range(n)]
    ],
    ...
    [(rootd[i][j] == -1) == shaded[i][j] for i in range(n) for j in range(n)],

    # ensuring conectedness (covering tree)
    [scp in T for i in range(n) for j in range(n) if (scp := rootd.cross(i, j), T := table(len(scp) - 1))]
)
