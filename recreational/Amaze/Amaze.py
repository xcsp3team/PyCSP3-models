...
free_cells = [(i, j) for i in range(n) for j in range(m) if (i, j) not in [tuple(p) for pair in points for p in pair]]

# x[i][j] is the value at row i and column j (possibly 0)
x = VarArray(size=[n, m], dom=range(len(points)))


# tables used for ensuring the presence of some neighbors
def T(r):  # r is the number of neighbors (2, 3 or 4)
    return [tuple([0] + [ANY] * r)] + [tuple([v] + [v if k in (i, j) else ne(v) for k in range(r)]) for v in Values for i, j in combinations(r, 2)]
...
satisfy(
    ...
    [x.cross(i, j) in T(len(x.beside(i, j))) for i, j in free_cells]
)
