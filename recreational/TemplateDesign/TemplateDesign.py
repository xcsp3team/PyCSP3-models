...
nSlots, demands = data or load_json_data("catfood-2.json")

nTemplates = nVariations = len(demands)
variation_intervals = [range(ceil(demands[v] * 0.95), floor(demands[v] * 1.1) + 1) for v in range(nVariations)]
...
if not variant():
    satisfy(
        ...
        p * d[:, j] in variation_intervals[j] for j in range(nVariations)
    )

elif variant("aux"):
    # pv[i][j] is the number of printings of the jth variation by using the ith template
    pv = VarArray(size=[nTemplates, nVariations], dom=lambda i, j: range(variation_intervals[j].stop))
    ...
    satisfy(
        ...
        [Sum(pv[:, j]) in variation_intervals[j] for j in range(nVariations)]
    )
