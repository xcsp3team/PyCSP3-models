...
assert variant("a2") or variant("a3")

distances = data or load_json_data("galaxy04.json")

nTeams, nRounds = len(distances), len(distances) * 2 - 2
assert nTeams % 2 == 0, "An even number of teams is expected"
nConsecutiveGames = 2 if variant("a2") else 3  # just used in one comment

T, R = range(nTeams), range(nRounds)


def Tb3(i):  # this is a table for the first or last game of the ith team
    return {(1, ANY, 0)} | {(0, j, distances[i][j]) for j in T if j != i}


def Tb5(i):  # this is a table for a game that is not the first or last one of the ith team
    return ({(1, 1, ANY, ANY, 0)} |
            {(0, 1, j, ANY, distances[i][j]) for j in T if j != i} |
            {(1, 0, ANY, j, distances[i][j]) for j in T if j != i} |
            {(0, 0, j1, j2, distances[j1][j2]) for j1 in T for j2 in T if different_values(i, j1, j2)})
