...
satisfy(
    ...
    [If(
        ...
        Then=[
            op[x[t][j], t],
            [If(i != x[t][ME], i != x[t][ELEPHANT], Then=op[i][t] == op[i][t - 1]) for i in Nodes],
            x[t][j] == x[t - 1][j]
        ],
        Else=[
            [If(x[t][j + 1] != i, Then=op[i][t] == op[i][t - 1]) for i in Nodes],
...
5) data used in 2023 are : 6,12,19,20,28
6) we can use x[t][j + 1]
 instead of x[t][(j + 1) % nPersons]
 because there is auto-adjustment of array indexing
"""
