...
if variant("mini"):
    Ts = [[(w, 1 if v == w else 0) for w in range(q)] for v in range(q)]

    T = [(u, v, w) for u in range(q) for v in range(q) for w in range(q) if not (u == v == w)]

    y = VarArray(size=[n, q], dom={0, 1})

    satisfy(
        [(x[i], y[i][v]) in Ts[v] for i in range(n) for v in range(q)],

        [Sum(y[:, v]) == L for v in range(q)],
    ...
    satisfy(
        ...
        Cardinality(x, occurrences={v: L for v in range(q)}),
