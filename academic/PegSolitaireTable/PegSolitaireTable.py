...
def table(i, j, t):
    V1 = [k for k, tr in enumerate(transitions) if (i, j) in (tr[0:2], tr[2:4], tr[4:6])]
    V2 = [k for k, tr in enumerate(transitions) if (i, j) in (tr[0:2], tr[2:4])]
    V3 = [k for k, tr in enumerate(transitions) if (i, j) == tr[4:6]]
    T = []
    for k in range(nTransitions):
        for v1 in (0, 1):
            for v2 in (0, 1):
                if not ((k not in V1) == (v1 == v2)):
                    continue
                if not ((k in V2) == (v1 == 1 and v2 == 0)):
                    continue
                if not ((k in V3) == (v1 == 0 and v2 == 1)):
                    continue
                T.append((k, v1, v2))
    return T
...
satisfy(
    ...
    [(y[t], x[t][i][j], x[t + 1][i][j]) in table(i, j, t) for (i, j) in pairs for t in range(nMoves)]
)

"""
1) Data used for the 2025 competition are: [(0,2,0), (0,3,0), (0,4,0), (1,2,0), (1,3,0), (1,4,0), (2,0,0), (2,2,0), (2,3,0), (2,4,0), (2,6,0), (3,3,0)]
"""
