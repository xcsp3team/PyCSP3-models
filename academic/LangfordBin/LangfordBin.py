...
From paper (by Gent, Jefferson and Miguel at CP'06]:
    Langford’s Problem L(k, n) requires finding a list of length k∗n, which contains k sets of the numbers 1 to n,
    ...
    Each variable in V, which represents the result, has domain {1, 2, ..., n}.
    For each i ∈ {1, 2, ...} the 2ith and 2i + 1st variables in P are the first and second positions of i in V.
...
satisfy(
    [v[p[2 * i]] == i + 1 for i in N],

    [v[p[2 * i + 1]] == i + 1 for i in N],

    [p[2 * i] == i + 2 + p[2 * i + 1] for i in N]
)
